var Overtime={
					...
					Overtime.objs.datagridFields.push(
						{title:"核定时长",field:"realOverHours",width:100,fixed:true,
							editor:{
						        type:"text",
						        "options":{
						        	rules:{required:true,number:true,min:0,max:99,rangelength:[0,6],messages:{rangelength:"长度介于0-6之间"}}
						        }
							},
							formatter:function(value,row,index){
								return value?AtdDataFormat.formatDoubleDays(value):0;
		...
		initBtn:function(){
			...
			$( "#commitButton" ).click(function(){
				...
				if( Overtime.objs.processInstId != null && Overtime.objs.processInstId != "" ){//非新建的提交包含了退回的操作
					...
					},dataCommit)),closeTab,null,null,null,null,/*Overtime.loadData*/function(){$("#commitButton").button("reset")});
				}else{//新建和启动流程
				//	$(this).button("loading");
		...
		calOvertimeHourTime:function(){
			...
			var count;
			if(Math.ceil(hour)-hour == 0.5){//等于0.5取0.5
				count = parseFloat(hour).toFixed(1);
			}else{
				count = Math.round(hour)>=hour?Math.round(hour):Math.round(hour)+0.5;
			}
			tbl.find("td[field='planOverHours']").find("input").val(count);
