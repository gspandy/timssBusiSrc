...
var modelFlag = false;
	...
	$(document).ready(function() {
		...
		dataGrid = $("#contentTb").iDatagrid("init",{
	        ...
	        singleSelect :true,
	        url: basePath + "attendance/abnormity/queryAbnormityBySiteId.do",	//basePath为全局变量，自动获取的       
	        "columns":[[
				...
				{field:'category',title:'异常类型',width:100,fixed:true,sortable: true,
				...
				{field:'createDate',title:'申请时间',fixed:true,width:150,sortable: true,
					formatter:function(val){
						return FW.long2time(val);
					}	
				},
				/* {field:'startDate',title:'考勤异常时间',fixed:true,width:150, sortable: true,
					formatter:function(val){
						return FW.long2time(val);
					}	
				}, */
				{field:'reason',title:'申请事由',width:100, sortable: true},
				{field:'status',title:'状态',width:120,fixed:true,sortable: true},
				/* {field:'active',title:'是否有效',width:150,
					formatter:function(val){
	        ...
	        onDblClickRow : function( rowIndex, rowData ){
		    	var url = basePath + "attendance/abnormity/detail.do?id=" + rowData.id + "&mode=edit";
		    	
		    	addTabWithTree( "updateAbnormity"+rowData.id, "考勤异常", url,"attendance", "contentTb" );
		    }

	    });
		
		$("#btn_add,#addBtn").click(function(){
			var url ="${basePath}attendance/abnormity/detail.do?mode=add";
			addTabWithTree( "addAttendance", "考勤异常", url,"attendance", "contentTb");
		});
		
		$("#itemUserInfo").iInput("init",{onSearch:function(){
			/* if($("#itemUserInfo").val()===""){
				FW.error("请输入要查询的异常人员的姓名或工号");
				return;
			} */
			modelFlag = true;
			if($(".itcui_btn_mag")[1]){
...
<body style="height: 100%;" class="bbox list-page">
	...
	<div id="grid1_empty" style="display:none;width:100%;height:62%">
		<div style="height:100%;display:table;width:100%">
			<div style="display:table-cell;vertical-align:middle;text-align:center">
			    <div style="font-size:14px">没有考勤异常信息</div>
