...
var fields = [
				...
				{title : "申请人", id : "createByStr",type:"label"},
	  			...
	  			/* {title : "申请中的天数", id : "aprDays",type:"label",formatter:formatterDays}, */ 
	  			...
	  			{title : "申请时间", id : "createDate",linebreak:true,type : "datetime",
						 dataType : "datetime"}
	  		];
		...
		var data = {
			...
			//当有审批权限 状态是草稿，说明还在申请阶段
			 if( userId == rowData.createBy  
					 && ( rowData.status == "草稿" || rowData.status == "提交请假审批表" ) ){
				$( "#closeDiv" ).hide();
				...
				if(rowData.status == "草稿" ){
					...
					$(".inner-title").html("编辑请假申请");
				}else if(rowData.status =="提交请假审批表" ){
					$("#deleteButtonDiv").hide();
					$("#invalidButtonDiv").show();
					$(".inner-title").html("编辑请假申请");
				}
			}else if( applyFlag =="approver" && rowData.status != "草稿"){
				auditInfoShowBtn = 1;
				$( "#closeDiv" ).hide();
				$( "#approveButtonDiv" ).show();
				$( "#fristNodeButtonDiv" ).hide();
				$("#deleteButtonDiv, #invalidButtonDiv").hide();
			}else if( rowData.status == "已归档" || rowData.status == "终止" 
					|| applyFlag =="others" || rowData.status == "作废"){
				$( "#closeDiv" ).show();
				$( "#approveButtonDiv" ).hide();
				$( "#fristNodeButtonDiv" ).hide();
				$("#deleteButtonDiv, #invalidButtonDiv").hide();
			...
			//初始化datagrid
			setLeaveApplyDatagrid( rowData.id );
			//附件
			initFile( rowData.id );
	...
	function initForm( id ){
		...
		var url = basePath + "attendance/leave/queryLeaveById.do?id=" + id;
		$.ajax({
			url : url,
			type : 'post',
			dataType : "json",
	...
	//删除一行datagrid数据
	function deleteNoteById( index ,caller){
		$("#leaveDatagrid").datagrid("deleteRow",index);
	}
	
	//设置明细datagrid
	function setLeaveApplyDatagrid( id ){
		...
		$("#leaveDatagrid").iDatagrid("init",{
	        ...
	        onLoadSuccess: function(data){
	        	if( !editFlag ){
		        	...
		        	$("#leaveDatagrid").datagrid("hideColumn","id");
		        	FW.fixRoundButtons("#toolbar");
	        	}else{
	        		$("#addDetailBtn_toolbar").show();
	        		$("#leaveDatagrid").datagrid("showColumn","id");
	        		var len = $("#leaveDatagrid").datagrid("getRows").length;
	        		FW.fixRoundButtons("#toolbar");
	    			for( var i = 0; i < len; i++ ){
	...
	function setApplyLeaveDays(){
		var datagridData = $("#leaveDatagrid").datagrid('getRows');
	...
	function audit(){
		...
		//workFlow.showAudit(taskId,dataArr,closeTab);
		workFlow.showAudit(taskId,  JSON.stringify( dataArr ), closeTab, closeTab, stop, "",1);
	}
	...
	$(document).ready(function() {
		...
		if($.inArray(siteId,printSite)<0){
			$("#printDiv").hide();
		}else{
			$( "#printBtn" ).click(function(){
				var url = fileExportPath + "preview?__report=report/TIMSS2_"+siteId+
											"_LEAVE_001_pdf.rptdesign&__format=pdf&id=" + businessId+"&siteid="+Priv.secUser.siteId;
				//window.open(url);
				FW.dialog("init",{src: url,
					...
					dlgOpts:{ width:800, height:650, closed:false, title:"打印请假单", modal:true }
				 });
			});
		}
		...
		$( "#commitButton" ).click(function(){
			...
			if( processInstId != null && processInstId != "" ){
				...
				var variables = [];
				variables.push({'name':'countDay','value':$("#autoform").iForm("getVal","leaveDays")});
				params['variables'] = FW.stringify(variables);
				$.post(url,params,function(data){
					if(data.result == 'ok'){
						workFlow.showAudit(taskId,JSON.stringify( dataArr ),closeTab,closeTab,stop,null,null,function(){
					...
					success : function(data) {
						if( data.result == "success" ){
							...
							if(taskId != null){
								...
								workFlow.submitApply(taskId, JSON.stringify( dataArr ),closeTab,null,0);
							}
						}else{
							if( data.reason != null ){
								FW.error( data.reason );
		...
		$("#flowDiagramBtn").click(function(){
			var workFlow = new WorkFlow();
			if( isNull( processInstId ) ){
				var flowKey = "atd_" + siteId.toLowerCase() + "_leave";
				showFlowDialog( flowKey );
			}else{
		...
		$( "#invalidButton" ).click(function(){
			...
			FW.confirm("确定作废本条数据吗？该操作无法恢复。", function() {
				var url = basePath + "attendance/leave/invalidLeave.do?id=" +  formData.id;
				$.ajax({
					url : url,
					type : 'post',
					dataType : "json",
	...
	function stop(){
	    ...
	    var url = basePath + "attendance/leave/deleteFlowLeave.do";
...
<div class="bbox toolbar-with-pager" id="toolbar_wrap">
	    <div id="toolbar" class="btn-toolbar " >
	        ...
	        <div class="atd_btn_pri atd_le_update_approve btn-group btn-group-sm" style="" id="approveButtonDiv">
				<button type="button" class="btn btn-default" id="approveBtn">审批</button>
	        </div>
	        <div class="atd_btn_pri atd_le_update_save btn-group btn-group-sm" style="display: none;" id="fristNodeButtonDiv">
	        ...
	        <div class="atd_btn_pri atd_le_insert_delete btn-group btn-group-sm" style="" id="deleteButtonDiv">
				<button id="deleteButton" type="button" class="btn btn-default" >删除</button>
	        </div>
	        <div class="atd_btn_pri atd_le_insert_invalid btn-group btn-group-sm" style="" id="invalidButtonDiv">
	        ...
	        <div id="printDiv" class="btn-group btn-group-sm">
	         ...
	         <div id="btn_flowDiagramDiv" class="atd_btn_pri atd_le_update_flow btn-group btn-group-sm">
	...
	<div grouptitle="请假天数明细 " id="detailTitle">
		<form class="margin-title-table" id="leaveFormDatagrid">
