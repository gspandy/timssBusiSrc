...
var fields = [
              {title : "免打卡部门",type : "label",id : "orgsName",wrapXsWidth : 12,wrapMdWidth : 12,formatter : joinTags}
	  		];
$(document).ready(function() {
	...
	$( "#updateButton" ).click(function(){
		...
		diffForm(f,"users");
		diffForm(f,"orgs");
		var url = basePath + "attendance/exempt/updateExempt.do";
		$.ajax({
			url : url,
			type : 'post',
			dataType : "json",
			...
			success : function(data) {
				if (data.result == "success") {
					FW.success("保存成功 ！");
					FW.navigate(basePath+"attendance/exempt/queryExemptAtdMenu.do");
				} else {
					FW.error("保存失败 ！");
				}
			}
...
<body>
	<div class="bbox toolbar-with-pager" id="toolbar_wrap">
	    <div id="toolbar" class="btn-toolbar">
	        ...
	        <div class="btn-group btn-group-sm" style="display: none;" id="userButtonDiv">
				<button type="button" class="btn btn-default" id="userButton">设置免打卡人员</button>
	        </div>
	        <div class="btn-group btn-group-sm" style="display: none;" id="orgButtonDiv">
				<button type="button" class="btn btn-default" id="orgButton">设置免打卡部门</button>
	        </div>
	...
	<div id="userDiv">
		<div  class="itcui_frm_grp_title" style="width:100%;">
			<span class="itcui_frm_grp_title_txt">免打卡人员</span>
		</div>
