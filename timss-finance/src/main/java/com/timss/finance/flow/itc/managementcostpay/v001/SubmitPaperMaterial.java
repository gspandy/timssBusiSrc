...
public class SubmitPaperMaterial extends TaskHandlerBase {
	...
	private static Logger logger = Logger.getLogger(SubmitPaperMaterial.class);
	
	public void init(TaskInfo taskInfo) {
		// 预留环节初始化时执行的方法
		String id = (String) wfs.getVariable( taskInfo.getProcessInstanceId(), WorkFlowConstants.BUSINESS_ID );
		logger.info("开始SubmitPaperMaterial流程时id:"+id);
		...
		if(StringUtil.isNotEmpty(mainId)){
			financeMainService.updateFinanceMainStatusByFid( "fmp_submit_paper_material", mainId );
		}
	}
	
	public void onComplete(TaskInfo taskInfo) {
		String id = (String) wfs.getVariable( taskInfo.getProcessInstanceId(), WorkFlowConstants.BUSINESS_ID );
		logger.info("完结SubmitPaperMaterial流程时id:"+id);
	}
