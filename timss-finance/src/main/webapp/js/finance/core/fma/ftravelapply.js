...
function initPageData(id){
	...
	function(data){
		if(data.flag == "success"){
			...
			var isTravel = formdata.isTravel;
			...
			if(_status=='D'){
				...
				$("#btn_delete").hide();
				$("#btn_obsolete").hide();
				if(loginUserId != formdata.createuser || isTravel != null){
					$("#btn_newTravelFin").hide();
				}
				if(isTravel != null){
					$("#form1").iForm('show',["isTravel"]);
				}else{
					$("#btn_cancel").hide();
				}
				initAttachForm(formdata.attachMap,$('#attachForm'),$('#attachFormWrapper'),true);
 ...
 * 新增才旅费报销
 */
function newTravelFinance(){
	var finTypeEn="only";
	var finNameEn = "travelcost";
	var data=$("#form1").iForm('getVal');
	var allowanceType = data.type;
	...
	var pageUrl =  basePath+"finance/financeInfoController/createPage.do?"
		...
		"&applyId="+id+"&allowanceType="+allowanceType;
	//$("#btn_cancel").hide();
	addEventTab( newId, pageName, pageUrl, oldId );
}
...
function cancelTravel(){
	$("#form1").iForm('setVal',{isTravel:"没有出差"});
	shenpi();
}
