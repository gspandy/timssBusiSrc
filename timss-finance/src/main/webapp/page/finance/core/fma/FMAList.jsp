...
<script>
	...
	$(document).ready(function() {
		...
		dataGrid = $("#test_grid1").iDatagrid("init",{
	        ...
	        columns : [ [
	         			...
	         			{field : 'subject',title : '列支科目',width : 100,align : 'left',
	         				fixed : true,sortable:true,
	         				formatter:function(value,row,index){
	         					var data=FW.parseEnumData("FIN_SUBJECT",_enum);
	        					var r="";
	        					for(var i in data){
	        						if(data[i][0]==value){
	        							r=data[i][1];
	...
	/* function addNewBidResultTab(){
		var newId="finNewFMATab";
		var pageName="新建管理费用申请";
		var pageUrl=basePath + "finance/fma/addFMAJsp.do";
		var oldId=FW.getCurrentTabId();
		addEventTab( newId, pageName, pageUrl, oldId );
	} */
