...
var updateFields = [
	...
	*/
	...
	function setUpdateBtnPriv( rowData ){
		...
		var newPtwflag = contains(newPtwPrivTypes,stdType); //登录用户是否有权限新建此类工作票，如果没有，隐藏生成工作票按钮
		if(!newPtwflag){
			$( "#newPtwButtonDiv" ).hide();
		}
		//当有审批权限 状态是草稿，说明还在申请阶段
		 if( currentUser == rowData.createuser  
			...
			$( "#closeBtn,#approveBtn,#modifyButtonDiv,#printButtonDiv,#newPtwButton" ).hide();
			$( "#editDiv,#saveButton,#commitButton,#flowDiagramBtn" ).show();
			editFlag = true;
			//草稿状态时
			...
			auditInfoShowBtn = 1;
			//审批状态
			$( "#closeBtn,#approveBtn,#flowDiagramBtn" ).show();
			$("#approveBtn,#flowDiagramBtn").show();
			/*$( "#editDiv,#deleteButtonDiv,#cancelButtonDiv,#expireButtonDiv,#modifyButtonDiv,#printButtonDiv,#newPtwButton" ).hide();*/
			...
			$( "#printButtonDiv,#flowDiagramBtn" ).show();
		...
		if(new Date().getTime() > rowData.endTime){
...
//		setPriv();
	...
	function setValidTime(){
		var sptwFormObj = $("#autoform").iForm("getVal");
		var beginTime = sptwFormObj.beginTime;
		var endTime = sptwFormObj.endTime;
		var sptwCode = sptwFormObj.wtNo;
	    ...
	    var btnOpts = [{"name" : "关闭",
			        	...
			        	},
			           {
			            ...
			            "onclick" : function(){
	        				...
	        				$.post(basePath + "ptw/ptwStandard/setValidTimeData.do", 
			                		{"id" : id,"beginTime":setDateForm.beginTime,
	    ...
	    var dlgOpts = {width : 300,height:200, title:"设置标准工作票有效时间"};
