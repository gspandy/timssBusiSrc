...
var fireBaseInfoField = [
	...
	{id:"fireWpExecNo",title : "动火执行人",type:"combobox",rules : {required:true},options:{allowSearch:true,allowEmpty:true,remoteLoadOn:"init",url:basePath+ "ptw/ptwInfo/queryPtwUsersByGroup.do?role=PTW_workpic_DY"}},
...
function initNewBaseInfo(){
	...
	if(ptwTypeCodes.length>0){
		//工作负责人修改
		var licWpicNoField = ptwUtil.findFieldById(baseInfoField,"licWpicNo");
		licWpicNoField.options.url = basePath+ "ptw/ptwInfo/queryPtwUsersByGroup.do?role=PTW_workpic_"+ptwTypeCodes[0]+"&prefix="+prefix;
	}
