...
//		}
		...
		$.ajax({
			...
			success : function(data) {
				...
				if(ptwStatus <= 300 || ptwStatus == 310){//草稿、未签发
					...
					$("#ptw_btn_print").show();
				}else if(ptwStatus == 410||ptwStatus == 420||ptwStatus == 430){//审核中
					if(result){
				...
				if(!(ptwTypeCode=='HY'||ptwTypeCode=='HE')){
					$("#ptw_btn_copy").show();
				}
...
function editPtwBaseInfoAndSafe(){
	isEditing = true;
	canSelectTree = true;
	if(ptwStatus == 300 || ptwStatus == 310){
		//按钮组全部隐藏
	...
	var editFields=["createUserNameAndTime","licWorkClassNum","licWorkClassPeople","licWorkClass",
	   	         ...
	   	         ,"eqName","wtTypeId","wtNo","isOutSourcing","woWindStation","licWpicNo"];
	   	
	$("#baseInfoForm").iForm("beginEdit",editFields);
	
	//处理是否编辑'是否外委'字段
	if(!outSourcingEdit){		
		$("#baseInfoForm").iForm("endEdit","isOutSourcing");
	}else{
		if(loginUserId != ptwCreateUser){			
			$("#baseInfoForm").iForm("endEdit","isOutSourcing");
		}
	...
	var fireEditFields = ["guardXmCo","fireWpExecNo","fireWc","fireUnit"];
...
function copyPtw(){
	...
	$("#baseInfoForm").iForm("setVal",{id:null,workOrderId:null,workOrderNo:null,keyBoxNo:null,isStdWt:0});
	isStdWt = false;
