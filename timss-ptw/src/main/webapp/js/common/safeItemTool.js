...
function endEditSafeItemList(outerDivId){	
	var tempdata = {};
	tempdata.safeDatas = getSafeItemInputs(outerDivId).safeItems;    //提取数据
...
function initSafeItemListByData(outerDivId,safeItemList){
	if(safeItemList){
		...
		for(var j = 0 ; j < safeDatas.length;j++){
			...
			$(append).find(".safe-input-content").html(FW.specialchars(safeDatas[j].content)).show();
...
function getSafeItemInputs(outerDivId){
	...
	for(var j = 0 ;j < inputs.length; j++){
		...
		if(tempValue){
			if(tempValue.length > 370){
				valid = false;
				invalidMsg = "部分安全措施超过了370个字符";
				return {valid:valid,invalidMsg:invalidMsg,safeItems:safeItems};
			}else {
				var safeType = null;
				 if(!(typeof($("#"+outerDivId).attr("safetype"))=="undefined")){ 
					//判断是否含有safetype属性
					 safeType = $("#"+outerDivId).attr("safetype");
				 } 
...
function showOperSafeItemListByData(outerDivId,safeItemList){
	if(safeItemList){
		...
		for(var j = 0 ; j < safeDatas.length;j++){
			...
			$(append).find(".safe-input").val(safeDatas[j].content).hide();
			//是否有执行
			var htmlCode = safeDatas[j].hasOper == 'Y'?"&radic;":"&times;";
			//$(append).find(".check-content").html(safeDatas[j].hasOper).show();
			$(append).find(".check-content").html(htmlCode).show();
			$(append).find(".check-input").hide();
			//执行时间
			$(append).find(".time-content").html(FW.specialchars(safeDatas[j].operTime)).show();
			//备注
			$(append).find(".safeRemark-input-content").html(FW.specialchars(safeDatas[j].remarks)).show();
...
/**
 * 操作步骤验证过滤
 * @param safeItems  操作步骤
 * @returns boolean
 */
function safeItemFilter(safeItems){
	//操作步骤验证
	var hasItemData = false;
	var safeDatas = [];
	var errorMsg = '';
	for(var i=0;i<safeItems.length;i++){
		var safeContent = $.trim(safeItems[i].content);
		if(safeContent !== ''){
			hasItemData = true;
			safeDatas.push(safeItems[i]);
		}
	}
	if(!hasItemData){
		FW.error("至少应该有一条操作步骤");
	}
	return {hasItemData:hasItemData,safeDatas:safeDatas};
}