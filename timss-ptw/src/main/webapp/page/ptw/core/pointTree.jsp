...
<script>
	...
	function initTree(){
		$('#asset_tree').tree({
			url : basePath + "ptw/islPoint/getAssetTree.do",
	...
	function initHint(){
		$("#search").ITCUI_HintList({
			"datasource":basePath + "ptw/islPoint/assetHint.do",
			"getDataOnKeyPress":true,
			"clickEvent":function(id,name){
				$("#search").val(name);
				$.ajax({
					url : basePath + "ptw/islPoint/assetParents.do?id=" + id,
					type:'POST',
					dataType:"json",
	...
	function changeToTreeNode( bean ){
		var obj={};
		obj.id=bean.assetId;
		obj.text=bean.assetName;
...
<body class="bbox" style="height:100%;width:100%">
	<div style="padding-top:33px;width:100%;height:100%">
		<div class="input-group input-group-sm bbox" style="width:210px;position:absolute;top:5px">
			<input type="text" placeholder="按名称或编号搜索" id="search" icon="itcui_btn_mag"/>     
		</div>
