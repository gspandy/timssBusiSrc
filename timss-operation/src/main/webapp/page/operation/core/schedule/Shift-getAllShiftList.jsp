...
<script>
	$(document).ready(function() {
		...
		dataGrid = $("#contentTb").iDatagrid("init",{
	        ...
	        singleSelect :true,
	        url: basePath + "operation/shift/queryAllShiftList.do",	//basePath为全局变量，自动获取的       
	        "columns":[[
					{field:'num',title:'班次编号',width:120,fixed:true,hidden:true},
					{field:'name',title:'名称',width:120,fixed:true},
					{field:'stationId',title:'工种',width:180,fixed:true},
					{field:'abbName',title:'班次别名',width:70,fixed:true},
					{field:'startTime',title:'开始时间',width:80,fixed:true},
					{field:'longTime',title:'上班时长',width:80,fixed:true},
					{field:'type',title:'班次类型',width:80,fixed:true,
	        ...
	        onDblClickRow : function( rowIndex, rowData ){
		    	var url = "${basePath}page/operation/core/schedule/Shift-updateShift.jsp?id=" + rowData.id;
		    	
		    	addTabWithTree( "shiftDetail"+rowData.id, "班次", url,"opmm", "contentTb" );
		    }

	    });
		
		$("#btn_add").click(function(){
			var url = "${basePath}page/operation/core/schedule/Shift-insertShift.jsp";
			addTabWithTree( "addShift", "班次", url,"opmm", "contentTb");
		});
		...
		$("#btn_advlocal").click(function() {
			if($(this).hasClass("active")){
				...
				},noSearchColumns:{4:true,5:true,6:true,7:true,8:true}});
			}
		});
