<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<title>行列规则列表</title>
<script>_useLoadingMask = true;</script>

<!-- 由于在本地web工程中，这个路径不存在，会编译异常 -->
<jsp:include page="/page/mvc/mvc_include.jsp" flush="false" />
<%-- <script type="text/javascript" src="<%=basePath %><%=resBase %>js/common/addTab.js?ver=${iVersion}"></script> --%>
<script src='${basePath}js/operation/common/addTab.js?ver=${iVersion}'></script>
<script>
	$(document).ready(function() {
		//此处一定要给dataGrid赋值。dataGrid为全局变量，不设置会导致不能resize
		dataGrid = $("#contentTb").iDatagrid("init",{
	        pageSize:pageSize,//pageSize为全局变量，自动获取的
	        singleSelect:true,
	        url: basePath + "operation/rules/queryAllRulesList.do",	//basePath为全局变量，自动获取的       
	        "columns":[[
						{field:'id',title:'ID',checkbox:true},
						{field:'num',title:'行列规则编号',width:120,fixed:true},
						{field:'name',title:'行列规则名称',width:150,fixed:true},
						{field:'dutyCount',title:'值别总数',width:80,fixed:true},
						{field:'period',title:'天次总数',width:80,fixed:true},
						{field:'changeLimit',title:'替换日期(每月)',width:120,fixed:true,
							formatter: 
								function(val,row){
									if( val == "0"){
										return "";
									}else{
										return val;
									}
								}
						},
						{field:'pollSequence',title:'轮询班次规则',width:120,fixed:true},
						{field:'available',title:'是否生效',width:80,
							formatter: 
							function(val,row){
								if( val == "Y"){
									return "是";
								}else{
									return "否";
								}
							}
						}
						]],
			"onLoadError": function(){
				//加载错误的提示 可以根据需要添加
				$("#grid1_wrap,#toolbar_wrap").hide();
			    $("#grid1_error").show();
			},
	        onLoadSuccess: function(data){
	            //远程无数据需要隐藏整个表格和对应的工具条，然后在无数据信息中指引用户新建
	            if(data && data.total==0){
	            	$("#grid1_wrap").hide();
	                $("#grid1_empty").show();
	            }else{
	            	$("#grid1_wrap").show();
	                $("#grid1_empty").hide();
	            }
	        },
	        onDblClickRow : function( rowIndex, rowData ){
		    	var url = "${basePath}page/operation/core/schedule/Rules-updateRules.jsp";
		    	FW.set("operationRowData",rowData);
		    	addTabWithTree( "rulesDetail"+rowData.id, "行列规则详情"+rowData.id, url,"opmm", "contentTb" );
		    }

	    });
		
		$("#btn_add").click(function(){
			window.location.href="${basePath}page/operation/core/schedule/Rules-insertRules.jsp";
		});
		
		//生成排版规则详细
		$("#createRulesDetail").click(function(){
			var rowData = $("#contentTb").datagrid("getSelected");
			//如果没有勾选数据，单选
			if( rowData == null || rowData == "" ){
				FW.error( "请勾选一条行列规则 ！" );
				return;
			}
			var pageUrl = "${basePath}page/operation/core/schedule/RulesDetail-insertRulesDetail.jsp";
			FW.set("operationRowData",rowData);
			addTabWithTree("rulesDetail" + rowData.id , "生成排班规则详情", pageUrl);
		});
		
		//高级搜索
		$("#btn_advlocal").click(function() {
			if($(this).hasClass("active")){
				$("#contentTb").ITCUI_GridSearch("end");
			}
			else{
				$("#contentTb").ITCUI_GridSearch("init",{"remoteSearch":true,"onParseArgs":function(arg){
					return {"search":JSON.stringify(arg)};
				}});
			}
		});
		
	});
</script>
</head>
<body style="height: 100%;" class="bbox list-page">
	<div class="bbox toolbar-with-pager" id="toolbar_wrap">
	    <!-- 这里可以在分页器的同排渲染一个按钮工具栏出来 在下面的toolbar1中 -->
	    <div id="toolbar1" class="btn-toolbar ">
	        <div class="btn-group-sm">
	           <button type="button" class="btn btn-success" id="btn_add">新增行列规则</button>
	           <button type="button" class="btn btn-default" id="createRulesDetail">生成排班规则</button>
	           <button type="button" class="btn btn-default" data-toggle="button" id="btn_advlocal">查询</button>
	        </div>
	    </div>
	    <!-- 上分页器部分 这里可以通过属性bottompager指定下分页器的DIV-->
	    <div id="pagination_1" class="toolbar-pager" bottompager="#bottomPager">        
	    </div>
	</div>
	<!--这里要清掉分页器的右浮动效果-->
	<div style="clear:both"></div>
	<div id="grid1_wrap" style="width:100%">
	    <table id="contentTb" pager="#pagination_1" class="eu-datagrid">
	    </table>
	</div>
	<!-- 下页器部分-->
	<div id="bottomPager" style="width:100%;margin-top:6px">
	</div>
	<!-- 错误信息-->
	<div class="row" id="grid1_error" style="display:none">
	    无法从服务器获取数据，请检查网络是否正常
	</div>
	<!-- 无数据 -->
	<div class="row" id="grid1_empty" style="display:none">
	    没有xxxxxxx数据，单击这里创建新的....
	</div>
</body>
</html>