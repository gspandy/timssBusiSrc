...
var printNoteReportName=null;
var noteDatagridHideColumns=["type",'writeTime','crewNum'];
var currentShiftStartTime = "0000";  //当班开始时间
var nowShiftDateLong = 0;  //当班日期（Long类型）
 
$(function(){
	//加载岗位和工种
 	OprNote.objs.deptList=${deptList};
	OprNote.objs.userJobsStr="${userJobsStr}";
	OprNote.objs.noteFormHideField=["preShiftMeeting","postShiftMeeting","dutyPerson"];
	OprNote.objs.opts.onlySchedulePersonJiaojieBan=false;
	OprNote.init();
	
	//定期轮询，查询当前班次是否有未完成的定期工作(每5分钟)
	setInterval("sendMessageRemind()",1000*60*5);
});

function sendMessageRemind(){
	var minutes = new Date().getMinutes();
	var minutesFlag = minutes>55 || minutes<5 ; //整点前后5分钟
	var hourFlag = new Date().getHours() > (Number(currentShiftStartTime)/100);   //大于当班的开始时间
	var threeHourFlag = ((new Date().getHours())/3 == 0)||((new Date().getHours()+1)/3 == 0);
	var timeFlag = minutesFlag && hourFlag && threeHourFlag;
	if( timeFlag){  //如果时间是在整点附近，大于当班的开始时间
		var currentShiftId = $("#baseform").iForm("getVal").currentShiftId;
		var date = $("#baseform").iForm("getVal").dateTime;
