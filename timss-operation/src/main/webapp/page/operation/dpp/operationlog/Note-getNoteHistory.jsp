	...
	function setNoteHistory( stationId, jobsId, shiftId, date ){
		...
		dataGrid = $("#contentTb").iDatagrid("init",{
	        ...
	        "columns":[[
	    				...
	    				{field:'dutyName',title:'值别',width:50,fixed:true}
	    				]],
			...
			onBeforeLoad : function() {
				$.each(["type",'writeTime','crewNum'],function(name,value){
	...
	function getBaseInfo(stationId,jobsId,shiftId,dateStr){
		...
		dataGrid2 = $("#contentTb2").iDatagrid("init",{
	        ...
	        "columns":[[
					...
					{field:'persons',title:'值班人员',width:200,hidden:true,formatter:function(value,row,index){
					...
					{field:'nextRemark',title:'交班交待',width:350,hidden:true},
					{field:'blank',title:' ',width:1}
				]],
			"onLoadError": function(){
				//加载错误的提示 可以根据需要添加
				$("#baseInfoDiv").hide();
