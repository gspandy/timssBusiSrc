...
public class MtpToNewTodo {
	...
	private static Logger logger = Logger.getLogger(MtpToNewTodo.class);
	...
	public void cycMtpToNewTodo() throws Exception{
		logger.debug("周期性维护计划生成工单定时任务扫描开始时间"+ new Date());
		List<MaintainPlan> mtpList = maintainPlanDao.queryAllCycMTP();
		 
		for(int i=0; i<mtpList.size(); i++){
			MaintainPlan tempMTP = mtpList.get(i);
			...
			if(flag){  //预警期已到
				...
				if(tempMTP.getIsAutoGenerWo()==1){  //设置了直接生成工单
					...
					if(tempMTP.getHasAlertTodo()==0){  //此周期内未生成提醒待办（防止重复生成提醒待办）
						...
						String jumpPath = "workorder/maintainPlan/queryFullMTPPage.do?todoId="+ProcessId+"&maintainPlanId="+tempMTP.getId();
				...
				logger.debug("周期（天）:--------------"+tempMTP.getMaintainPlanCycle());
