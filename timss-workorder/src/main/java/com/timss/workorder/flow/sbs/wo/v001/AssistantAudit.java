...
public class AssistantAudit extends TaskHandlerBase {
	@Autowired
    ...
    public void init(TaskInfo taskInfo){
    	...
    	String woStatus = "assistantAudit";
    ...
    public void onComplete(TaskInfo taskInfo){
    	...
    	logger.debug("-------------进入‘场长助理审核’的onComplete(),开始处理业务逻辑-----------------");
...
//    	String processInstId = workflowService.getVariable(taskInfo.getProcessInstanceId(), "processInstId").toString();
		...
		String handlerStyle = handlerStyleFormDataJson.getString("isNowHandlerAssistant");
		
		if("laterHandlerAssistant".equals(handlerStyle)){ //如果是不立即处理，则入库维护计划列表
			WorkOrder workOrder;
		...
		}else if("nowHandlerAssistant".equals(handlerStyle)){
		...
		handStyleMap.put("attribute", "IS_NOWHANDLER_ASSISTANT");
