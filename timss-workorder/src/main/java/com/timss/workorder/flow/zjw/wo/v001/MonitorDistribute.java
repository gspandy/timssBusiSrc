...
public class MonitorDistribute  extends TaskHandlerBase {
	@Autowired
	...
	private static Logger logger = Logger.getLogger(MonitorDistribute.class);
	...
	public void init(TaskInfo taskInfo){
        ...
        logger.info( "MonitorDistribute init --- instantId = " + instantId + "-- businessId = " + woId );
        //更新工单状态
        woUtilService.updateWoStatus(woId, WoProcessStatusUtilZJW.MONITOR_DISTRIBUTE);
        super.init( taskInfo );
	}
    ...
    public void onComplete(TaskInfo taskInfo){
        String instantId = taskInfo.getProcessInstanceId();
        String woId = workflowService.getVariable(taskInfo.getProcessInstanceId(), "businessId").toString();
        logger.info( "MonitorDistribute onComplete --- instantId = " + instantId + "-- businessId = " + woId );
    ...
    public void beforeRollback(TaskInfo taskInfo, String destTaskKey){
        String instantId = taskInfo.getProcessInstanceId();
        String woId = workflowService.getVariable(taskInfo.getProcessInstanceId(), "businessId").toString();
        logger.info( "MonitorDistribute beforeRollback --- instantId = " + instantId + "-- businessId = " + woId );
		//修改处理人
        UserInfoScope userInfoScope = itcMvcService.getUserInfoScopeDatas();
