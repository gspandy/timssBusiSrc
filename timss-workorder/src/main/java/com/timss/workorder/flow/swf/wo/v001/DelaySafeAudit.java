...
public class DelaySafeAudit  extends TaskHandlerBase{
	@Autowired
	...
	private static Logger logger = Logger.getLogger(DelaySafeAudit.class);
	...
	public void init(TaskInfo taskInfo){
        ...
        logger.info( "DelaySafeAudit init --- instantId = " + instantId + "-- businessId = " + woId );
        //更新工单状态
        woUtilService.updateWoStatus(woId, WoProcessStatusUtil.DELAY_SAFE_AUDIT_STR);
        super.init( taskInfo );
	}
    ...
    public void onComplete(TaskInfo taskInfo){
        String instantId = taskInfo.getProcessInstanceId();
        String woId = workflowService.getVariable(taskInfo.getProcessInstanceId(), "businessId").toString();
        logger.info( "DelaySafeAudit onComplete --- instantId = " + instantId + "-- businessId = " + woId );
		//修改处理人
    ...
    public void beforeRollback(TaskInfo taskInfo, String destTaskKey){
        String instantId = taskInfo.getProcessInstanceId();
        String woId = workflowService.getVariable(taskInfo.getProcessInstanceId(), "businessId").toString();
        logger.info( "DelaySafeAudit beforeRollback --- instantId = " + instantId + "-- businessId = " + woId );
		//修改处理人
        UserInfoScope userInfoScope = itcMvcService.getUserInfoScopeDatas();
