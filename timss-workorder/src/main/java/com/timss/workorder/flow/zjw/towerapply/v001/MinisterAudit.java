...
public class MinisterAudit extends TaskHandlerBase {
    @Autowired
    ...
    private static Logger logger = Logger.getLogger(MinisterAudit.class);
    ...
    public void init(TaskInfo taskInfo){
      ...
      //更新登塔申请状态
        woTowerApplyDao.updateTowerApplyStatus(towerApplyId, TowerApplyStatusUtil.MINISTER_AUDIT);
        super.init( taskInfo );
    }
    ...
    public void onComplete(TaskInfo taskInfo){
        ...
        String towerApplyId = workflowService.getVariable(taskInfo.getProcessInstanceId(), "businessId").toString();
        String cabinRoof = workflowService.getVariable(taskInfo.getProcessInstanceId(), "cabinRoof").toString();
        //修改当前处理人
        Map<String, String> parmas = new HashMap<String, String>();
        parmas.put( "towerApplyId", towerApplyId );
        if("N".equals(cabinRoof)){
            parmas.put( "currHandler", "" );
            parmas.put( "currHandlerName", "" );
            woTowerApplyDao.updateTowerApplyStatus(towerApplyId, TowerApplyStatusUtil.FINISH);
        }else{
	        try {
