...
public class ShieldDutyReset  extends TaskHandlerBase {
	@Autowired
	...
	private static Logger logger = Logger.getLogger(ShieldDutyReset.class);
	...
	public void init(TaskInfo taskInfo){
        ...
        logger.info( "ShieldDutyReset init --- instantId = " + instantId + "-- businessId = " + woId );
        //更新工单状态
        woUtilService.updateWoStatus(woId, WoProcessStatusUtilZJW.SHIELD_DUTY_RESET);
        super.init( taskInfo );
	}
    ...
    public void onComplete(TaskInfo taskInfo){
        String instantId = taskInfo.getProcessInstanceId();
        String woId = workflowService.getVariable(taskInfo.getProcessInstanceId(), "businessId").toString();
        logger.info( "ShieldDutyReset onComplete --- instantId = " + instantId + "-- businessId = " + woId );
		//修改处理人
    ...
    public void beforeRollback(TaskInfo taskInfo, String destTaskKey){
        String instantId = taskInfo.getProcessInstanceId();
        String woId = workflowService.getVariable(taskInfo.getProcessInstanceId(), "businessId").toString();
        logger.info( "ShieldDutyReset beforeRollback --- instantId = " + instantId + "-- businessId = " + woId );
		//修改处理人
        UserInfoScope userInfoScope = itcMvcService.getUserInfoScopeDatas();
