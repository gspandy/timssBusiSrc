...
/* 表单字段定义  */
	var fields = [
		    ...
		    {title : "维护组",id : "principalGroup", type : "combobox",
		    	rules : {required:true},
		    	options : {
			    		url : basePath + "workorder/woUtil/userGroupFilter.do?filterStr=itc_wo_ft",
			    		allowEmpty : true,
			    	    remoteLoadOn : "init"
			    	}	
		    },
	...
	$(document).ready(function() {	
		$.ajaxSetup({"async":false});  //设置为顺序执行
		 ...
		 if(faultTypeId == null || faultTypeId == 'null'){ 
			...
			$.post(basePath + "workorder/woParamsConf/queryFaultTypeDataById.do",{"faultTypeId":faultTypeId},
	...
	**/
	function saveFaultType(){
		...
		$.post(basePath + "workorder/woParamsConf/commitFaultType.do",
	...
	*/
	function deleteFaultType(){
		Notice.confirm("确定删除|确定删除此故障类型以及其子类型么？",function(){
			$.post(basePath + "workorder/woParamsConf/deleteFaultType.do",
