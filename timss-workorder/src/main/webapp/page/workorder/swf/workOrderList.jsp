...
<script>
	...
	var overWoFilterFlag = 0 ;
	var unDoneFilterFlag = 0;
	//表格初始化
	function initDataGrid(){
		dataGrid = $("#workorder_table").iDatagrid("init",{
			...
			queryParams: {overWoFilterFlag:function(){return overWoFilterFlag;},unDoneFilterFlag:function(){return unDoneFilterFlag;}},  			    
			columns:[[
				{field:"workOrderCode",title:"编号",width:150,fixed:true,sortable:true},
				{field:"woSpecCode",title:"专业",width:70,fixed:true,sortable:true,
				...
				{field:"urgentDegreeCode",title:"紧急",width:40,fixed:true,sortable:true,
					formatter: function(value,row,index){
				...
				{field:"currStatus",title:"状态",width:120,fixed:true,sortable:true,
					formatter: function(value,row,index){
			...
			onDblClickRow : function(rowIndex, rowData) {
				var tabId;
				if(rowData.currStatus=="DRAFT" || rowData.currStatus=="WO_COMMIT" || rowData.currStatus=="DUTY_CONFIRM_DEFECT"){
					tabId = "workOrderAdd";
				}
				else{
					tabId = "workOrderInfo";
				...
				var opts = {
					id : tabId + rand,
					name : "工单",
					url : basePath+ "workorder/workorder/openWorkOrderAddPage.do?woId=" + woId,
					tabOpt : {
						closeable : true,
	...
	function rowStyler(rowIndex, rowData) {
		 if (rowData.isOverWo == 'Y' && rowData.currStatus != "DONE" && rowData.currStatus != "OBSELETE") {//超时 未完成  有效
				return 'color:red;';
		 } 
	}
	//页面初始化
	...
	function newWO(){
		...
		var urlPath = basePath+ "workorder/workorder/openWorkOrderAddPage.do?woStatus=draft&equipParams="+encodeURIComponent(equipParams);
		var opts = {
	...
	//过滤超时工单
	 function filterOverWo(){
		overWoFilterFlag = 1;
		unDoneFilterFlag = 0;
		refresh();
	}
	//过滤未完成工单
	 function filterUnEndWo(){
	 	overWoFilterFlag = 0;
		unDoneFilterFlag = 1;
		refresh();
	}
	...
	<div class="bbox toolbar-with-pager" id="toolbar_wrap">
		...
		<div id="toolbar1" class="btn-toolbar ">
			<div class="btn-group btn-group-sm">
				<button type="button" class="btn btn-success priv" privilege="WO_NEW" onclick="newWO()">新建</button>
			</div>
			<div class="btn-group btn-group-sm">
				<button type="button" id="btn_advSearch" data-toggle="button" class="btn btn-default">查询</button>
			</div>
			<div class="btn-group btn-group-sm">
				<button type="button" id="btn_filter" class="btn btn-default dropdown-toggle" data-toggle="dropdown">工单过滤
					<span class="caret"></span> <!-- 添加文字右边的"下箭头" -->
				</button>
				<ul class="dropdown-menu">
					<li><a onclick="filterOverWo()">超时工单</a></li>
					<li><a onclick="filterUnEndWo()">未完成工单</a></li>
				</ul>
			</div>
		</div>
