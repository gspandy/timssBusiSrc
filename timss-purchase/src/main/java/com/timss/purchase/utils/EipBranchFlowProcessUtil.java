...
public class EipBranchFlowProcessUtil {
  private static final Logger LOG = Logger.getLogger(EipBranchFlowProcessUtil.class);
  ...
  public boolean setFlowVariablesBeforeCommit(String processId, String taskDefKey) throws Exception {
    // 根据流程实例id和流程定义key获取流程节点属性
    Map<String, String> elementMap = getFlowElementProperty(processId, taskDefKey);
    // 解析流程节点信息，并设置流程变量
    if (elementMap != null) {
      String modifiable = elementMap.get("modifiable");
      // 解析modifiable节点
      if (StringUtils.isNotBlank(modifiable)) {
        Map<String, Object> modifiableMaps = null;
        try {
          ObjectMapper objectMapper = new ObjectMapper();
          modifiableMaps = objectMapper.readValue(modifiable, Map.class);
        } catch (Exception e) {
          throw new Exception("流程定义id为" + processId + ",taskDefKey为" + taskDefKey
              + "的流程配置modifiable有错误", e);
        }
        // 设置流程变量
        setFlowVariablesByFlowProperty(modifiableMaps, processId);
      }
    }
    return true;
  }
  ...
  // 根据流程实例id和流程定义key获取流程节点属性
  private Map<String, String> getFlowElementProperty(String processId, String taskDefKey) {
    // 根据流程实例id获取流程定义key
    String processDefKey = workflowService.getDefKeyByProcessInstId(processId);
    // 获取节点配置信息
    return workflowService.getElementInfo(processDefKey, taskDefKey);
  }

  // 根据流程配置设置流程变量
  private void setFlowVariablesByFlowProperty(Map<String, Object> modifiableMaps, String processId) {
    ...
    if (setBranchVariables != null) {
      ...
      if (setBranchVariables instanceof Map) {
        ...
        setFlowVariableByVariableMap(processId, setBranchVariables);

      } else {
        LOG.warn("流程id为" + processId + "的setBranchVariables属性值不是对象,可能导致分支节点不生效");
      }
    }
  ...
  // 根据具体流程变量map，设置工作流信息
  @SuppressWarnings("unchecked")
  private void setFlowVariableByVariableMap(String processId, Object setBranchVariables) {
  ...
  // 为分支节点获取所以可能的下一节点
  @SuppressWarnings("unchecked")
  private void getAllNextElementForBranch(Map<String, Object> modifiableMaps, String processId,
      List<RetTask> rtList, Task currentTask) throws Exception {
      ...
      if (branchVariables instanceof Map) {
        ...
        for (Map.Entry<String, Object> entry : branchVariableMaps.entrySet()) {
          ...
          if (value != null && value instanceof Map) {
            // 设置具体流程变量
            setFlowVariableByVariableMap(processId, value);
            setNextLinkBean(processId, currentTask, rtList);
          } else {
            throw new Exception("流程定义id为" + processId
                + "的流程配置modifiable有错误,是流程分支节点但是变量信息branchVariables格式出错");
          }
        }
        // 检查下一节点是否设置正确
        ...
        throw new Exception("流程定义id为" + processId
            + "的流程配置modifiable有错误,是流程分支节点但是确没有配置变量信息branchVariables");
      }
    }
  }

  /**
   * @description: 排除掉重复的环节项
   * @author: 890166
   * @createDate: 2015-4-1
   * @param rtList
   *          :
  ...
  private void checkNextElements(List<RetTask> rtList) {
    ...
    int counter = 0;
    int dyncCounter = rtList.size();
    for (int i = 0; i < dyncCounter; i++) {
      RetTask rt = rtList.get(i);
      strSet.add(rt.getTask().getKey());
      if (counter < strSet.size()) {
        counter = strSet.size();
      } else {
        rtList.remove(i);
        dyncCounter = rtList.size();
      }
    }
  }
  ...
  private Boolean validBranchElement(Map<String, Object> modifiableMaps, String processId) {
    ...
    if (isBranchElement != null) {
      if (isBranchElement instanceof Boolean) {
        ...
        LOG.warn("流程id为 " + processId + " 的isBranchElement属性值不是boolean对象,是否配置错误");
      }
    }
  ...
  // 设置下一节点信息
  public void setNextLinkBean(String processId, Task task, List<RetTask> rtList) {
    // 获取下一节点定义id
    ...
    if (null != nkeyList && !nkeyList.isEmpty()) {
      for (String nextKey : nkeyList) {
        ...
        TaskDefInfo tdi = workflowService.getTaskDefInfoByTaskDefKey(pi.getProcessDefinitionId(),
            nextKey);
        rt.setTask(new RetKeyValue(tdi.getDefKey(), tdi.getName()));

        // 找到下一个节点的人员信息
        List<RetKeyValue> rkvList = new ArrayList<RetKeyValue>();
        List<SecureUser> suList = workflowService.selectUsersByTaskKey(processId, nextKey,
            task.getId());
