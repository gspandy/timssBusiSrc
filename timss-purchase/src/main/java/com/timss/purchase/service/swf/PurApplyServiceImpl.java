...
/**
 * @title: 采购申请不采用关联待办表来查询待办人
 * @description: 采购申请不采用关联待办表来查询待办人
 * @company: gdyd
 * @className: PurApplyServiceImpl.java
 * @author: 890166 Copy by 890162
...
public class PurApplyServiceImpl extends PurApplyServiceDefImpl {
    ...
    public String checkStopable(String sheetId){
        ...
        if ( 0==result.length()&&purApplyItems2.size()>0 &&0==result.length() ) {
    ...
    @Override
    public boolean stopPurApply(String sheetId) {
    ...
    @SuppressWarnings("unused")
    @Override
    public boolean startStopPurApply(String sheetId,String processInstId) {
        boolean result = false;
    ...
    @SuppressWarnings("unused")
    @Override
    public boolean nullifyStopPurApply(String sheetId) {
        boolean result = false;
        UserInfoScope userInfo = itcMvcService.getUserInfoScopeDatas();
        PurApply purApply = purApplyDao.queryPurApplyBySheetId( sheetId );
