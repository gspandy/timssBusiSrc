...
var PurApplyPriv={
		...
		set:function(){//定义权限
			...
			//打印
			Priv.map("privMapping.applypurch_print","applypurch_print");
			//新建领料单
...
function assetNatureChange(val){
	var rows = $("#apply_item").datagrid("getRows");
	var len = rows.length;
	for(var i=0;i<len;i++){
		$("#apply_item").datagrid("deleteRow",0);
	}
}
function projectAscriptionChange(val){
	var rows = $("#apply_item").datagrid("getRows");
	var len = rows.length;
	for(var i=0;i<len;i++){
		$("#apply_item").datagrid("deleteRow",0);
