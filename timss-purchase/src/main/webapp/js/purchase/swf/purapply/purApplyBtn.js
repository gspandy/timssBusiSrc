var ApplyBtn={
	...
	add:function(){
		$("#btn-add").click(function(){
			currOpFilter = {};
			itemFilter = null;
			FW.showInventoryDialog({categoryname:$("#f_itemclassid").parent().find(".cb_text").text(),
				active: "Y",
				onParseData : function(data){
					var cateType = $("#f_itemclassid").parent().find(".itcui_combo_text").text()
					var needAddOne = {};
					if(!itemFilter){
						itemFilter = {};
						var listData =$("#apply_item").datagrid("getRows");
					...
					for(var i=0;i<data.length;i++){
						//无仓库信息的物资 或选择了物资类型不对的物资 跳过 --开始
						var binid = data[i].binid;
						var warehouseid = data[i].warehouseid;
						var invcateid = data[i].invcateid;
						var itemCateType = data[i].cateType;
						if(null==binid||null==warehouseid){
							if(null==binid||null==warehouseid){
								FW.error("物资未关联仓库");
							}
	...
	edit:function(){
		...
		/*	if(processStatus != "last"){
			...
			var listData =$("#apply_item").datagrid("getRows");
			//SWF--表格可编辑权限控制--开始
			var gridEditable = ""!=oper?JSON.parse(oper).editable:"";//流程环节定义可否编辑
			if(!gridEditable){
				$("#apply_item").datagrid("hideColumn","del");
				for(var i=0;i<listData.length;i++){
