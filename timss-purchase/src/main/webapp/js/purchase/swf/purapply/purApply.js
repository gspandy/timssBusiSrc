var newMatApplyPriv=function(){
	if("true"==isContainsInQty&&"true"==isCreator){
		return true;
	}else{
		return false;
	}
...
var PurApplyPriv={
		...
		set:function(){//定义权限
			...
			//提交商务网
			Priv.map("privMapping.applypurch_tobuss","applypurch_tobuss");
			//新建
			Priv.map("privMapping.applypurch_newSingle","applypurch_newSingle");
			//删除
			Priv.map("privMapping.applypurch_delete","applypurch_delete");
			//新建领料单
			Priv.map("newMatApplyPriv()","applypurch_newMatApplyPriv");
			//执行情况
...
function sheetClassidChange(val){
    if("editable" == isEdit && ("draft"==process || ""==process)){
    	if("SCLWZ"!=val){
        	$("#autoform").iForm("hide",["major"]);
        }else{
        	$("#autoform").iForm("show",["major"]);
        }
        if("劳保用品"==val){
        	$("#autoform").iForm("setVal",{"assetNature":"NOT_ASSET"});
        	$("#autoform").iForm("endEdit","assetNature");
        }else{
        	$("#autoform").iForm("beginEdit","assetNature");
        }
    }
}
function itemClassidChange(val){
	//切换物品类型时，清空物资列表
	if(initForm){
		var rows = $("#apply_item").datagrid("getRows");
		for(var i=rows.length-1;i>=0;i--){
			$("#apply_item").datagrid("deleteRow",$('#apply_item').datagrid('getRowIndex',rows[i]));
		}
		$("#autoform").iForm("setVal",{"tatolcost":0});
	}
	$("#f_itemclassid").val(val);
	initForm = true;
}
function projectClassidChange(val){
	//技改项目、科技项目、特殊修理
	if("change"==val||"techChange"==val||"specilAssert"==val){
		$("#autoform").iForm("show","projectCode");
	}else{
		$("#autoform").iForm("hide","projectCode");
	}	
}
...
function caluPrice(){
	...
	if(undefined !=$("#apply_busi").html()&&(""!=$("#apply_busi").html()||$("#apply_busi").prev().length>0)){
		var busilistData =$("#apply_busi").datagrid("getRows");
		listData = listData.concat(busilistData);
	}
...
function dynaCalcTotalPrice(){
	...
	for(var i=0;i<rows.length;i++){
		...
		repliednum = parseFloat(repliednum);
...
//自动生成采购单--由于没有执行采购的环节，此方法目前生物质站点未被使用
function autoGenerateOrder() {
	...
	var url = basePath
			+ "purchase/purapply/autoGenerateOrder.do?appSheetId="
			+ sheetId + "&itemIds=" + itemIds;
...
//自动将所有物资生成采购合同--由于没有执行采购的环节，此方法目前生物质站点未被使用
function autoGenerateOrderWithAllItems() {
	var itemIds = null;
	var listData = [];
	var selectData = $("#apply_item").datagrid("getRows");
	if (null != selectData) {
		for ( var i = 0; i < selectData.length; i++) {
			listData.push(selectData[i].itemid);
		}
