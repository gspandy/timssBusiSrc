...
function commitApply(obj){
	var type = obj.type;
	var isFormal = obj.isFormal;
	var listData = null;
	
	if (type == "submit") {
		$("#btn-submitlist").button('loading');
	}
...
function autoGenerateForm(){
	var columns = [[
	        		...
	        		{field:'createUserName',width:90,fixed:true,title:'申请人'},
	        		...
	        		{field:'tax',width:60,title:'税费(元)',hidden:true},
	...
	for(var i=0;i<size;i++){
		...
		row["applyDept"] = dataList[i]["applyDept"];
		row["projectAscription"] = dataList[i]["projectAscription"];
