...
function initPayInfo(inForm){
	...
	$.ajax({
		...
		success : function(data) {
			...
			if("plancontractdept_manager"==process&&"true"==isCandidate){
				$("#autoform").iForm("beginEdit","countersign");
			}
...
function commitApply(obj){
	...
	var countersign = $("#autoform").iForm("getVal","countersign");
	if("plancontractdept_manager"==process){
		if ("" == countersign){
			FW.error("请选择是否会签");
			return;
		}
			...
			if( data.result == "success" ){
				if( "submit" == saveType ){
					...
					if("plancontractdept_manager"==process){
						...
						var params={};
						params['processInstId'] = procInstId;
						var variables = [{'name':'countersign','value':countersign}];
...
function printRpt(type){
	...
	if("title"==type){
		title = "打印报账封面";
		reportfile = "TIMSS2_ZJW_PAYTITLE_001";
	}else if("content"==type){
		title = "打印入库报账单";
		reportfile = "TIMSS2_ZJW_PAYCONTENT_001";
	}else if("qa"==type){
		title = "打印质保金审批表";
		reportfile = "TIMSS2_ZJW_PAYAUD_001";
	}else if("accept"==type){
		title = "打印固定资产领用单";
		reportfile = "TIMSS2_ZJW_PAYACCEPT_001";
	}
