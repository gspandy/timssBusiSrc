var ApplyBtn={
	...
	revert:function(){
		$("#btn-revert").click(function(){
			var forbid = $("#btn-revert").attr("forbid");
			if("true"==forbid){
				return;
			}
			var obj = {};
			...
			//新增本模块独有的纯退回的方法
			FW.showRevert(taskId,null,submitCallBack,pageClose,stopProcess,proMsg,0,cancelCallBack);
		});
	},
	//暂存按钮
	save:function(){
	...
	add:function(){
		$("#btn-add").click(function(){
			var assetNature = $("#autoform").iForm("getVal","assetNature");
			var projectAscription = $("#autoform").iForm("getVal","projectAscription");
			if(""==assetNature||""==projectAscription){
				FW.error("请先选择"+(""==assetNature?"资产性质":"")+(""==assetNature&&""==projectAscription?"和":"")+(""==projectAscription?"项目名称":""))
				return;
			}
			currOpFilter = {};
			itemFilter = null;
			FW.showInventoryDialog({
				isFixedAsset: "fixedAsset"==assetNature?"Y":"N",
				warehouseid: projectAscription,	
				active: "Y",
	...
	nobusiness:function(){
		$("#btn-nobusiness").click(function() {
			var forbid = $("#btn-nobusiness").attr("forbid");
			if("true"==forbid){
				return;
			}
	...
	print:function(){
		var url = fileExportPath+"preview?__format=pdf&__report=report/TIMSS2_ZJW_PA_001_pdf.rptdesign&siteid="+siteId+"&sheetid="+sheetId;
	...
	stop:function(){
		if(""!=stopStatus){
			$("#btn-stop").html("<a href=\"javascript:void(0)\">审批终止</a>");
		}
