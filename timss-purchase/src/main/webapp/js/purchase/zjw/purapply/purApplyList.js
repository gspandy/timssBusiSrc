...
function initInnerTab(){
	$("#tab_applyitem").on('shown.bs.tab', function (e) {
		...
		if("editable" == isEdit){
			for(var i =0 ;i<$("#apply_item").datagrid("getRows").length;i++){
	...
	$("#tab_busi").on('shown.bs.tab', function (e) {
		...
		if("first_save"!=processStatus&&"first"!=processStatus&&"process"!=processStatus){
	...
	$("#apply_busi").datagrid({
		...
		onLoadSuccess : function(data) {
			...
			if (data && data.total == 0) {
				...
				if (Priv.hasRole("ZJW_CGY")||Priv.hasRole("ZJW_CGZZ")) {
