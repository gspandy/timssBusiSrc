	...
	if ("" == sheetId) {
		...
		if (processStatus == "last") {
			...
			if ("editable" != isEdit) {
				...
				if("true"==allItemsApplying){
					FW.enableLi('btn-revert');
				}
			}
...
//over状态下的特殊处理
function specialTreatInOverState(){
	//有明细处于发送商务网 且 处于over 状态，且是最后一步处理人打开，关闭退回功能 
	if("over"==processStatus&&("true"==hasItemsBusinessApplying&&"true"==isLastStepAssignee)){
		FW.showBtnGroup("btn-group");
		$("#btn-revert").hide();
	}
	//有明细处于发送商务网 且 处于over 状态，且是采购员或专责角色打开，开放撤回商务网功能
	if ("true"==hasItemsBusinessApplying&&"over"==processStatus&&(Priv.hasRole("ZJW_CGY")||Priv.hasRole("ZJW_CGZZ"))) {
		FW.enableLi('btn-nobusiness');
	}
}
