 ...
 */
@Service
public class ReceiptToEipMobile implements EipMobileInterface{
	@Autowired
	...
	@Autowired
	PayDao payDao;
	private static final Logger LOGGER=Logger.getLogger(ReceiptToEipMobile.class);
	...
	public RetContentBean retrieveWorkflowFormDetails(ParamDetailBean eipmobileparambean) {
        ...
        LOGGER.info("eip接口获取收款信息，processInstId"+processId+",flowNo:"+flowNo);
	...
	private List<RetContentInLineBean> assembleFroms(UserInfoScope userInfo,
		...
		retContentInLineBean.setName("收款信息");
		retContentInLineBean.setValue(getPayMessage(payDtlVo));
		
		contentInLineBeans.add(retContentInLineBean);
		
		RetContentInLineBean invoices=getInvoice(payDtlVo);
		if(invoices!=null){
			contentInLineBeans.add(invoices);
		}
	...
	private RetContentInLineBean getInvoice(PayDtlVo payDtlVo) {
		RetContentInLineBean invoices=null;
		List<InvoiceVo> invoiceVos=payDtlVo.getInvoices();
		if(invoiceVos!=null && invoiceVos.size()!=0){
			...
			invoices.setName("发票信息");
			invoices.setValue(getInvoicesMessage(invoiceVos));
		}
		return invoices;
	}
	private List<Object> getInvoicesMessage(List<InvoiceVo> invoices) {
		List<Object> results=new ArrayList<Object>();
		for(int i=0;i<invoices.size();i++){
			HashMap<String, Object> map=new HashMap<String, Object>();
			InvoiceVo invoiceVo=invoices.get(i);
			map.put("title", invoiceVo.getSum());
			map.put("rows", toInvoiceMap(invoiceVo));
			results.add(map);
		}
		return results;
	}
	private Object toInvoiceMap(InvoiceVo invoiceVo) {
		List<Object> projectMaps=FlowEipUtil.getReturnForm(invoiceVo, "itc-invoice-table");
		return projectMaps;
