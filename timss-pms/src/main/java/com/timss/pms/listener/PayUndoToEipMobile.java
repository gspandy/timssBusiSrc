...
/**
 * @ClassName:     PayUndoToEipMobile
 * @company: gdyd
 * @Description:退票与eip接口
 * @author:    谷传伟
 * @date:   2015-09-01 
 */
@Service
@EipAnnotation("pms_itc_payundo")
public class PayUndoToEipMobile implements EipMobileInterface{
	@Autowired
	...
	@Autowired
	PayDao payDao;
	private static final Logger LOGGER=Logger.getLogger(PayUndoToEipMobile.class);
	...
	public RetContentBean retrieveWorkflowFormDetails(ParamDetailBean eipmobileparambean) {
		...
		// 表单的sheetNo
                ...
                LOGGER.info("eip接口获取退票信息，processInstId"+processId+",flowNo:"+flowNo);
                UserInfoScope userInfo = itcMvcService.getUserInfoScopeDatas();
                // 通过flowNo找到对应的id
                String id=payService.queryBusinessIdByUndoFlowId( processId );
	...
	private List<RetContentInLineBean> assembleFroms(UserInfoScope userInfo,
		...
		retContentInLineBean.setName("退票信息");
	...
	private List<Object> getPayMessage(PayDtlVo payDtlVo ) {
		...
		List<PayplanVo> payplanVos=payDtlVo.getPayplanVos();
		payDtlVoTmp.setPayStage(getPayStageById(payplanVos, payDtlVo.getPayplanId().toString()));
		
		List<Object> projectMaps=FlowEipUtil.getReturnForm(payDtlVoTmp, "itc-payundo-form");
		return projectMaps;
	}
		...
		RetProcessBean emrb = new RetProcessBean();
                ...
                try {
                    if ( "next".equals( flowId ) ) {// 提交下一环节
                        HashMap<String, Object> nextUserMap=new HashMap<String,Object>(0);
