 ...
 */
@Service
public class SelectUserForXGBM implements SelectUserInterface{
	@Autowired
	WorkflowService workflowService;
	@Autowired
	IAuthorizationManager iAuthorizationManager; 
	private static final Logger LOGGER=Logger.getLogger(SelectUserForXGBM.class);
	@Override
	public List<SecureUser> selectUsers(SelectUserInfo selectUserInfo) {
		LOGGER.info("开始获取流程"+selectUserInfo.getProcessInstId()+"的相关部门审批节点审批人员");
		//获取流程实例id
		...
		LOGGER.info("获取流程"+selectUserInfo.getProcessInstId()+"的相关部门审批节点人员所属的用户组是"+groupId);
		//获取用户组对应的所有用户
