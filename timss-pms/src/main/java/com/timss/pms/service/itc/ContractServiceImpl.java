package com.timss.pms.service.itc;
...
public class ContractServiceImpl implements ContractService {
	...
	private HashMap startWorkflow(Contract contract){
		...
		wBusiness.setBusinessId(WFServiceImpl.contractPrefix+String.valueOf(contract.getId()));
		wBusiness.setInstanceId(processInstId);
		wfbBusinessDao.insertWorkflowBusiness(wBusiness);
		//绑定合同到待办页面
		//changeContractStatus用于标识是否显示变更计划
		String url="pms/contract/editContractJsp.do?contractId="+contract.getId()+"&processInstId="+processInstId+"&changeContractStatus=1";
		homepageService.createProcess(wBusiness.getId(), processInstId, "结算计划变更", "合同-" + contract.getName(), "提出申请", url, infoScope, null);
	...
	public int updateContract(Contract contract,List<Payplan> payplans) {
		...
		//附件绑定
		AttachUtil.bindAttach(attachmentMapper, null, contract.getAttach());
		homepageService.Delete(createIdInHome(contract.getId()), itcMvcService.getUserInfoScopeDatas());
...
//		contractDtlVo.setBidId(bidDtlVo.getBidId());
	...
	public void deleteContract(String contractId) {
		...
		//去除首页的草稿记录
		homepageService.Delete(contract.getContractCategory(), itcMvcService.getUserInfoScopeDatas());
	...
	@Override
	public int stopWorkflow(Contract contract, String processInstId,String reason) {
		return 0;
	}
	@Override
	...
	private HashMap<String,Object> startAppWorkflow(Contract contract){
		UserInfoScope infoScope=itcMvcService.getUserInfoScopeDatas();
		String defkey = workflowService.queryForLatestProcessDefKey("pms_itc_contractapp");
		HashMap<String,Object> map=new HashMap<String,Object>(0);
		map.put("businessId", contract.getId());
		ProcessInstance processInstance=null;
		//根据用户所属部门是否为科技公司经营部 设置流程变量
		String currOrgId=infoScope.getOrgId();
		//经营部部门编号 1232605
		if("1232605".equals(currOrgId)){
			map.put("isManagerDept", "Y");
		}else{
			map.put("isManagerDept", "N");
		}
		map.put("needCounterSign", "N");
		map.put("needDeputyManagerCounterSign", "N");
		map.put("needChairmanSign", "N");
	...
	//更新不同类型的状态-end
        ...
        public Map<String, Object> startNullifyWorkflow(Contract contract) throws Exception{
                LOGGER.info("开始提交作废合同数据");
                UserInfoScope infoScope=itcMvcService.getUserInfoScopeDatas();
                String defkey = workflowService.queryForLatestProcessDefKey("pms_itc_contractnullify");
                HashMap<String,Object> map=new HashMap<String,Object>(0);
                map.put("businessId", contract.getId());
                Contract updateContract = contractDao.queryContractById( contract.getId() );
                ProcessInstance processInstance=null;
                String processInstId = updateContract.getNullifyProcInstId();
                //创建流程
                if(StringUtils.isEmpty( processInstId )){
                    try{
                        String isLarge = "false";
                        if ( contract.getTotalSum()>1000000 ) {
                            isLarge = "true";
                        }
                        map.put( "isLarge", isLarge );
                        processInstance = workflowService.startLatestProcessInstanceByDefKey(defkey,infoScope.getUserId(),map);
                    }catch(Exception e){
                        LOGGER.error( "--startNullifyWorkflow Has Exception:", e );
                        throw new PmsBasicException("合同作废审批流程启动失败", e);
                    }
                    processInstId = processInstance.getProcessInstanceId();
                    //保存流程实例id与businessid的关联
                    updateContract.setNullifyProcInstId( processInstId );
                    contractDao.updateContract( updateContract );
                    //创建作废合同审批的待办信息
                    String url="pms/contract/editContractJsp.do?contractId="+contract.getId()+"&processInstId="+processInstId+"&type=modify";
                    HomepageWorkTask homepageworktask=new HomepageWorkTask();
                    homepageworktask.setFlow(contract.getTipNo());
                    homepageworktask.setProcessInstId(processInstId);
                    homepageworktask.setUrl(url);
                    homepageworktask.setTypeName("作废合同");
                    homepageworktask.setName("作废合同-"+contract.getName());
                    homepageworktask.setStatusName("申请作废合同");
                    homepageService.create(homepageworktask, infoScope);
                }
                //获取当前活动节点，根据当前活动节点获取下一个任务节点、以及执行人，并返回给前台
                List<Task> activities = workflowService.getActiveTasks(processInstId);
                Task task = activities.get(0);
