...
public class WFServiceImpl implements WFService{
	...
	private Map<String,Object> createWFMap(String businessId,String processInstId,String taskId){
		...
		if(taskId==null){
			...
			if(inStringList(currentUser, users)){
			    ...
			    tmpMap = workflowService.getElementInfo(taskId);
			    if(tmpMap.containsKey( "modifiable" )){
			        if ( StringUtils.isNotEmpty( tmpMap.get( "modifiable" ) ) ) {
			            Map<String, String> modifyMap = JsonHelper.fromJsonStringToBean( tmpMap.get( "modifiable" ), HashMap.class );
	                            if ( null!=modifyMap&&modifyMap.containsKey( "formHide" ) ) {
	                                map.put( "formHide", modifyMap.get( "formHide" ) );
	                            }
                                }
			    }
                        }
	...
	public Map<String,Object> createContractMap(ContractDtlVo contractDtlVo,
		...
		Map<String,Object> map=createWFMap(contractPrefix+businessId, processInstId, taskId);
		return map;
	}
    ...
    public Map<String, Object> createContractAppNullifyMap(ContractDtlVo contractDtlVo) {
        Map<String,Object> map=new HashMap<String,Object>();
        String processInstId=contractDtlVo.getNullifyProcInstId();
        String taskId  = null;
        String isCandidate="false";
        if(StringUtils.isNotBlank(processInstId)){
            List<Task> tasks=workflowService.getActiveTasks(processInstId);
