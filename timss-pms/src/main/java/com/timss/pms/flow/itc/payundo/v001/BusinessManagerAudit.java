 ...
 * 退票申请 经营部经理审批节点类
 * 
 * @ClassName: BusinessManagerAudit
 * @company: gdyd
 * @Description:TODO
 * @author: 谷传伟
 * @date: 2015-08-28 
 */
public class BusinessManagerAudit extends TaskHandlerBase {
    @Autowired
    ...
    private static final Logger LOGGER = Logger.getLogger( BusinessManagerAudit.class );
    ...
    public void init(TaskInfo taskInfo) {
        ...
        if ( pay != null ) {
            ...
            pay.setUndoStatus( "businessManagerAudit" );
            InitUserAndSiteIdUtil.initUpdate( pay, itcMvcService );
            payService.updateStatus( pay );
        }
        LOGGER.info( "初始化退票经营部经理审批节点" + taskInfo.getTaskDefKey() + "对结算信息信息的更新" );
    }
    @Override
    public void onComplete(TaskInfo taskInfo) {
        LOGGER.info( "完成退票经营部经理审批节点" + taskInfo.getTaskDefKey() + "对结算信息信息的更新" );
    }
