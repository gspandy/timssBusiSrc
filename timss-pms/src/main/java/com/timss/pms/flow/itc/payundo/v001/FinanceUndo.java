 ...
 * 退票申请 财务退票节点类
 * 
 * @ClassName: FinanceUndo
 * @company: gdyd
 * @Description:TODO
 * @author: 谷传伟
 * @date: 2015-08-28 
 */
public class FinanceUndo extends TaskHandlerBase {
    @Autowired
    ...
    private static final Logger LOGGER = Logger.getLogger( FinanceUndo.class );
    ...
    public void init(TaskInfo taskInfo) {
        ...
        if ( pay != null ) {
            ...
            pay.setUndoStatus( "financeUndo" );
            InitUserAndSiteIdUtil.initUpdate( pay, itcMvcService );
            payService.updateStatus( pay );
        }
        LOGGER.info( "初始化退票财务退票节点" + taskInfo.getTaskDefKey() + "对结算信息信息的更新" );
    }
    @Override
    public void onComplete(TaskInfo taskInfo) {
        ...
        if ( pay != null ) {
            pay.setStatus( ChangeStatusUtil.voidedCode );
            pay.setUndoStatus( "pass" );
            InitUserAndSiteIdUtil.initUpdate( pay, itcMvcService );
            payService.updateStatus( pay );
        }
        LOGGER.info( "完成退票财务退票节点" + taskInfo.getTaskDefKey() + "对结算信息信息的更新" );
    }
}
