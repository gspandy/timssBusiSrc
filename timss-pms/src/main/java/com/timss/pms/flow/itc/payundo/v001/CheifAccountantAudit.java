 ...
 * 退票申请 主办会计审批节点类
 * 
 * @ClassName: CheifAccountantAudit
 * @company: gdyd
 * @Description:TODO
 * @author: 谷传伟
 * @date: 2015-08-28 
 */
public class CheifAccountantAudit extends TaskHandlerBase {
    @Autowired
    ...
    private static final Logger LOGGER = Logger.getLogger( CheifAccountantAudit.class );
    ...
    public void init(TaskInfo taskInfo) {
        ...
        if ( pay != null ) {
            ...
            pay.setUndoStatus( "cheifAccountantAudit" );
            InitUserAndSiteIdUtil.initUpdate( pay, itcMvcService );
            payService.updateStatus( pay );
        }
        LOGGER.info( "初始化退票主办会计审批节点" + taskInfo.getTaskDefKey() + "对结算信息信息的更新" );
    }
    @Override
    public void onComplete(TaskInfo taskInfo) {
        LOGGER.info( "完成退票主办会计审批节点" + taskInfo.getTaskDefKey() + "对结算信息信息的更新" );
    }
}
