...
<script src="${basePath}js/pms/itc/invoice/editInvoice.js?ver=${iVersion}"></script>
...
<script>
	...
	function submitData(form){
		...
		$.post(basePath+'pms/invoice/checkReceipt.do',{"invoice":FW.stringify(data),"invoiceConfirm":FW.stringify(gridData),"isInvoiceReceiptable":pmsPager.isInvoiceReceiptable},function(result){
			showBasicMessageFromServer(result,"确认到账成功","确认到账失败");
		});
	}
	...
	function validConfirmCheckListAmount(form){
		...
		if(totalRecordList<= pmsPager.invoiceSum){
			...
			if(totalRecordList==pmsPager.invoiceSum){
				FW.confirm("实际到账总金额等于发票金额，确认完结此发票已经完成收款？",{
				    onConfirm : function(){
...
<body>
	<div class="bbox toolbar-with-pager" id="toolbar_wrap">
	    <div id="toolbar1" class="btn-toolbar ">
	        ...
	        <div class="btn-group btn-group-sm ">
	            <button type="button" class="btn btn-default pms-button"  onclick="confirmCheck();" id="b-confirm">添加到账记录</button>
	        </div>
	...
	<div  class="margin-group-bottom" style="display:none;" id="addReceiptWrapper">
		<form id="receiptListWrapper" grouptitle="实际到账信息" class="margin-title-table">
