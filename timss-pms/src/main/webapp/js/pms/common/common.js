...
var garbageColunms={
    ...
    formatter:function(value,row,index){
	     return '<img src="'+basePath+'img/pms/btn_garbage.gif" title="删除当前行" width="16" height="16" >';
	}
...
//	if(readOnly){
//		window.noOnChange = true;
//	}
	form.iForm("init",{"fields":formFields,options:{validate:true,initAsReadonly:readOnly}});
	if(data && data.data){
...
//	//强制取消附件上传的限制,采用默认值
	if(attachFormFields && attachFormFields[0] && attachFormFields[0].options && attachFormFields[0].options.fileSizeLimit){
		delete attachFormFields[0].options.fileSizeLimit;
	}
//	$.extend(true,attachFormFields,extendFormFields);
//	//强制取消附件上传的限制,采用默认值30M
...
//		}
//	];
//	attachFormFields=$.extend(true,attachFormFields,extendFormFields);
...
function pmsPrintHelp(buttonId,url,title){
	FW.initPrintButton(buttonId,url,title);
}
/**
 * 初始化打印按钮
 ...
 * @despection
 */
function initPrintButton(buttonId,url,title,extParams){
...
//查看所属项目
FW.showProjectInfo = function(projectNameTargetId){
	if(hasMenuPriv("pmspmsproject")){
		$("div[fieldid='"+projectNameTargetId+"']").each(function(){
		      var spans = $(this).find("span");
		      for(var i = 0;i<spans.length;i++){
		    	  if("form-required"!=$(spans[i]).attr("class")){
		    		  $(spans[i]).wrap("<a onclick='openViewProject();' style='display:block;width:100%'></a>");	  
		    	  }
		      }
		});
	}
...
//查看合同 
FW.showContractInfo = function(contractNameTargetId){
	if(hasMenuPriv("pmspmscontract")){
		$("div[fieldid='"+contractNameTargetId+"']").each(function(){
			  var spans = $(this).find("span");
		      for(var i = 0;i<spans.length;i++){
		    	  if("form-required"!=$(spans[i]).attr("class")){
		    		  $(spans[i]).wrap("<a onclick='viewContract();' style='display:block;width:100%'></a>");	  
		    	  }
		      }  
		});
	}
...
function viewBid(){
	var id = $("#f_bidId").val();
	var idstr='pmsViewBidTab'+id;
	openTab(basePath+"pms/bid/editBidResultJsp.do?id="+id,'招标',idstr);
}
//查看所属招标
FW.showBidInfo = function(bidNameTargetId){
	if(hasMenuPriv("pmspmsbr")){
		$("div[fieldid='"+bidNameTargetId+"']").each(function(){
		      var spans = $(this).find("span");
		      for(var i = 0;i<spans.length;i++){
		    	  if("form-required"!=$(spans[i]).attr("class")){
		    		  $(spans[i]).wrap("<a onclick='viewBid();' style='display:block;width:100%'></a>");	  
		    	  }
		      }
		});
	}
};
function hasMenuPriv(id){
	if(privMapping[id]==1){
		return true;
	}else{
		return false;
