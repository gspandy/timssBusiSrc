...
var bidFormFields=[
	    ...
	    {id:"bidResultId",type:"hidden"},
	    {id:"projectId",type:"hidden"},
	    {id:"supplierId",type:"hidden"},
	    ...
	    {title : "预中标价(元)", id : "budget",
	    	rules:{
	    		required:true,
	    		number:true
	    	}
	    },
	    {title : "中标推荐单位1", id : "supplier"
	    },
	    {title : "中标推荐单位2", id : "supplier2Name"
	    },
	    {title : "副总经理审批", id : "needFZSH",
	    	type:"combobox",
	    	data:[["Y","是"],["N","否",true]],
	    	rules:{
	    		required:true
...
function initRemoteSupplierData(form){
	var $supplierInput = $('#f_supplier');
	var $supplierIdInput = $('#f_supplierId');
	var supplierInit = {
	...
	$supplierInput.iHint('init', supplierInit);
}
...
function valid(){
    ...
    var formValue=$form.iForm("getVal");
    var supplier2Name=formValue.supplier2Name;
    var supplier1Name=formValue.supplier;
    if(supplier2Name && supplier2Name==supplier1Name){
    	FW.error("推荐中标单位名称不能相同");
    	return false;
    }
	return true;
}
