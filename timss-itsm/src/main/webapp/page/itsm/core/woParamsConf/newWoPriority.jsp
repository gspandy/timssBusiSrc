...
var	woPriConfGridField = [[
		...
		{field : 'influenceScope',title : '影响范围',width:200,fixed:true,
			editor:{ 
					type : 'combobox',
					options : {
						data : FW.parseEnumData("ITSM_INFLUENCE_SCOPE",_enum),
						rules:{
							required:true
						}
					}
				...
				formatter:function(value){
					return  getEnumName("ITSM_INFLUENCE_SCOPE",value);
				}
		}, 
		{field : 'urgentDegree',title : '紧急度',width:200,fixed:true,
			editor:{ 
				...
				formatter:function(value){
					return  getEnumName("ITSM_URGENCY_DEGREE",value);
				}
		},
	...
	$(document).ready(function() {
		 ...
		 if(woPriorityId == null || woPriorityId == 'null'){
				...
				$.post(basePath + "itsm/woParamsConf/queryWoPriorityDataById.do",{"woPriorityId":woPriorityId},
					function(woPriorityData){
						...
						var woPriConfigObj = JSON.parse( woPriorityData.datagridData );
	...
	**/
	function savePriority(){
		...
		if(hasConfigOtherPri.woPriId != 0){
			var  influence = getEnumName("ITSM_INFLUENCE_SCOPE",hasConfigOtherPri.influenceVal);
			var  urgent = getEnumName("ITSM_URGENCY_DEGREE",hasConfigOtherPri.urgentVal);
			...
			if(woPriorityId==""){  
		 	...
		 	$.post(basePath + "itsm/woParamsConf/commitWoPriority.do",
	...
	function checkHasConfigData(woPriConfData){
		$.post(basePath + "itsm/woParamsConf/checkHasConfigData.do",
	...
	**/
	function deletePriority(){
		Notice.confirm("确定删除|确定删除此服务级别么？",function(){
			$.post(basePath + "itsm/woParamsConf/deletePriority.do",
			 		{"priId":woPriorityId},
					function(data){
						if(data.result == "success"){
							FW.success("删除成功")
...
<body style="height: 100%;" class="bbox">
	<div id="mainContent">
		...
		<div  class="inner-title">
			<span id="inPageTitle">服务级别详情</span>
		</div>
		<div>
