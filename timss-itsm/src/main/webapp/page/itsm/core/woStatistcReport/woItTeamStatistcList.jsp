...
<script>
	...
	$(document).ready(function(){
		...
		$("#selectTimeForm").iForm("setVal",{"beginTime":beginTime,"endTime":endTime})
 		...
 		dataGrid = $("#woItPersonStatistic_table").iDatagrid("init",{
	        ...
	        url: basePath + "itsm/woStatis/woItMaintainKPIData.do",     
	        queryParams:{
	        	statisticType:"teamStatistic",  //personStatistic、teamStatistic、
	        	begin:beginTime.getTime(),
	        	end:endTime.getTime()+24*60*60*1000
	        },
	        singleSelect:true,
	        columns:[[ 
					{field:"serLevel",title:"团队",width:100,align:'center',fixed:true,
	...
	function mergeDatagridCells(serLevelSum, mergeNumList){
		var beginIndex = 0;
		for(var i=0 ;i<serLevelSum; i++){
			$("#woItPersonStatistic_table").datagrid('mergeCells', {
				index: beginIndex,
				field: "serLevel",
				rowspan: mergeNumList[i],
				type: "body"
			});
			beginIndex = beginIndex + mergeNumList[i];
		} 
	}
	...
	function printStatisticList(){
		...
		var src = fileExportPath + "preview?__report=report/TIMSS2_ITSM_ITTEAM_KPI.rptdesign&__format=pdf"+
...
<body>
	...
	<!-- <div id="advSearchBar" style="height: 35px;margin-top: 6px;display: none;" class="bbox">
	    ...
	    <div class="btn-group btn-group-sm" style="float:left;margin-left:30px">
