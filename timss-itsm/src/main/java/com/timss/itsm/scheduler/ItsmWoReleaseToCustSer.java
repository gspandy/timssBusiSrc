...
@Component
@Lazy(false)
public class ItsmWoReleaseToCustSer {
	@Autowired
	...
	@Autowired
	ItsmWoPriorityService woPriorityService;
	@Autowired
    private IAuthorizationManager authManager;
	
	private static final Logger LOG = Logger.getLogger(ItsmWoReleaseToCustSer.class);
	

	@Scheduled(cron = "0 0/5 * * * ?")  //定时到每5分钟扫描一次
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor ={Exception.class})
	public void releaseWoByOvertime() throws Exception{
		LOG.debug("工单自动释放定时任务扫描开始时间"+ new Date());
		//有优先级，工单处于“workPlan”状态
		List<ItsmWorkOrder> woList = workOrderDao.queryAllWoNoSiteId();
		
		for(int i=0; i<woList.size(); i++){
			ItsmWorkOrder tempWo = woList.get(i);
			String woId = tempWo.getId();  //工单Id
			int priorityId = tempWo.getPriorityId(); //优先级Id
			String siteid = tempWo.getSiteid();
			Date sendWoTime = tempWo.getSendWoTime(); //派单时间
			Date nowTime = new Date();
			ItsmWoPriority woPriority = (ItsmWoPriority) woPriorityService.queryWoPriorityById(priorityId,siteid).get("baseData");
			...
			if(flag){  //满足了自动释放条件
				...
				LOG.debug("工单自动释放定时任务满足条件开始自动释放"+tempWo.toString());
				String siteId = tempWo.getSiteid();
				...
				List<SecureUser> tempList = authManager.retriveUsersWithSpecificRole(siteId+"_ITSM_KF", null, false, true);
