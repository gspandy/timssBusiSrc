		...
		if(userInfoScope!=null){
			...
			if(!"".equals(complainRd.getWorkflowId())){	
              //更新待办列表
                ...
                int size = (complainRd.getContent().length() > 100) ? 100 : complainRd.getContent().length();
                String flowName = complainRd.getContent().substring(0, size);
                ...
                HomepageWorkTask homeworkTask = woUtilService.joinHomepageWorkTask
                		(flowCode, flowName, complainRd.getWorkflowId(), "审批",  "意见和建议", jumpPath);
