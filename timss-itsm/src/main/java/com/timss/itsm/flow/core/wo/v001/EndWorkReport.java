...
public class EndWorkReport extends TaskHandlerBase {
    ...
    public void onComplete(TaskInfo taskInfo) {
        ...
        String jobPlanId = String.valueOf( workOrder.getJobPlanId() );

        // 操作附件
        try {
            woUtilService.insertAttachMatch( woId , attachmentIds, "WO", "endWorkReport" );
        } catch (Exception e) {
            LOG.error( e.getMessage() );
            throw new RuntimeException( e );
        }
        ...
        if ( jpPlanItemsList != null && jpPlanItemsList.size() > 0 ) {
