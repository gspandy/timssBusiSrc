...
<script>
	...
	$(document).ready(function() {
		$("#purorder_grid").iDatagrid("init",{
			singleSelect:true,
			pageSize:pageSize,
			url: basePath+"inventory/invmattran/queryPurOrderList.do",
	...
	function getPurOrderNos(){
		...
		if( rowData == null || rowData == "" ){
			...
			for(var i=0;i<rowData.length;i++){
				arr[i]=rowData[i].sheetno+"||"+rowData[i].sheetname+"||"+rowData[i].applyTypeName;
			}
			info = arr.join(",");
		}
		return info;
	    ...
	    <table id="purorder_grid" pager="#pagination" class="eu-datagrid">
	    	<thead>
				<tr>
					...
					<th data-options="field:'applyTypeName',hidden:true">采购类型</th>
				</tr>
			</thead>
	    </table>
	</div>
