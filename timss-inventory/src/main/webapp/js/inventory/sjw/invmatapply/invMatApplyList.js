...
function initList(){
	...
	var columns = [[
			...
			{field:'cusmodel',title:'型号规格',width:140,fixed:true},
			{field:'warehouse',title:'仓库',width:90,fixed:true},
			{field:'invcate',title:'物资类别',width:90,fixed:true},
			{field:'nowqty',title:'可用库存',width:70,align:'right',fixed:true,formatter:function(value){
				return parseFloat(value).toFixed(2);
			}},
			{field:'stockqty',title:'实际库存',width:70,align:'right',fixed:true,hidden:true,formatter:function(value){
				return parseFloat(value).toFixed(2);
			}},
			{field:'qtyApply',title:'申领数量',width:70,align:'right',fixed:true,edit:true,
				editor:{
					type:'text',
					options:{
						align:"right",
			...
			{field:'unit1',title:'单位',width:40,fixed:true},
			...
			{field:'totalprice',title:'含税金额',width:80,align:'right',fixed:true,
				formatter:function(value){
		    		return parseFloat(value).toFixed(2);
				}	
			},
			...
			{field:'totalNoTaxPrice',title:'不含税金额',width:80,align:'right',fixed:true,
				formatter:function(value){
		    		return parseFloat(value).toFixed(2);
				}	
			},
	...
	var cscolumns = [[
			...
			{field:'cusmodel',title:'型号规格',width:140,fixed:true},
			{field:'warehouse',title:'仓库',width:90,fixed:true},
			{field:'bin',title:'货柜',width:90,fixed:true},//多出来的
			{field:'invcate',title:'物资类别',width:90,fixed:true},
			...
			{field:'qtyApply',title:'申领数量',width:70,align:'right',fixed:true},
			{field:'outqty',title:'已发料',width:60,align:'right',fixed:true},
	...
	$("#matapplydetail_grid").datagrid({
        ...
        onAfterEdit:function(rowIndex, rowData){
        	...
        	var noTaxPrice = rowData.noTaxPrice;
			
			var rowTotal =null;
			var rowNoTaxTotal =null;
			var alreadyOut = null;
			...
			if(flag){
				...
				if(processStatus != "last"){
					...
					rowNoTaxTotal = parseFloat(qtyApply * noTaxPrice).toFixed(2);
					
					alreadyOut = (outqtytemp*1+outstockqty*1);
					$("#matapplydetail_grid").datagrid("updateRow",{
						index: rowIndex,
						row: { "totalprice": Number(rowTotal),"totalNoTaxPrice": Number(rowNoTaxTotal),"outqty":alreadyOut,"outqtytemp": outqtytemp}
					});
				}
			}
		},
       ...
       success:function(data){
           	...
           	if(null != result && "" != result){
                ...
                for(var i=0;i<resultArr.length;i++){
                	...
                	totalNTP = parseFloat(rowJ.noTaxPrice).toFixed(2);//沙C多经的价格是严格区分的
					...
					row["noTaxPrice"] = totalNTP;
					row["totalNoTaxPrice"] = totalNTP;
					row["outqty"] = '0';
					row["outqtytemp"] = '';
					row["outstockqty"] = '0';
					...
					totalNTPrices = parseFloat(totalNTPrices*1 + totalNTP*1).toFixed(2);
                }
            	...
            	var total = {totalPrice:totalPrices,totalNoTaxPrice:totalNTPrices,dept:deptName};
