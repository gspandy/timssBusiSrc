package com.timss.inventory.flow.sbs.invmatapply.v001;
...
public class Draft extends TaskHandlerBase {
    ...
    public void init(TaskInfo taskInfo) {
        ...
        // 拿到“经理”角色的编码
        String bz = CommonUtil.getProperties( "bz" );
        ...
        if ( null != roList && !roList.isEmpty() ) {
            for ( Role role : roList ) {
                ...
                if ( bz.indexOf( role.getId() ) > -1 && !bzFlag ) {
                    bzFlag = true;
                }
            }
        }
        ...
        if ( bzFlag ) {
            workflowService.setVariable( processId, "isSquad", "Y" );
        } else {
            workflowService.setVariable( processId, "isSquad", "N" );
        }
    }
