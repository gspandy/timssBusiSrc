 ...
 * @description: {desc}
 * @company: gdyd
 * @className: InvMatApplyToEipMobile.java
 * @author: 890166
 * @createDate: 2014-9-24
 * @updateUser: 890166
 * @version: 1.0
 */
@EipAnnotation("invMatApply")
@Component
public class InvMatApplyToEipMobile implements EipMobileInterface {
    ...
    private static final Logger LOG = Logger.getLogger( InvMatApplyToEipMobile.class );
    ...
    public RetContentBean retrieveWorkflowFormDetails(ParamDetailBean eipmobileparambean) {
        ...
        try {
            ...
            String imaid = invMatApplyService.queryImaIddByFlowNo( flowNo, userInfo.getSiteId() );

            // 组装forms中数据
            List forms = assembleContent( userInfo, imaid, processId );
            ...
            // 组装flows中数据
            /*String processDefKey = getPropertiesVal( "invmatapplyKey" );
            ...
            throw new RuntimeException(
                    "---------InvMatApplyToEipMobile 中的 retrieveWorkflowFormDetails 方法抛出异常---------：", e );
        }
        return efdb;
    }
     ...
     * @return:
     */
    private List<RetContentInLineBean> assembleContent(UserInfoScope userInfo, String imaid, String processId)
            throws Exception {
        ...
        // 收集主单信息
        List<InvMatApplyVO> imaList = invMatApplyService.queryMatApplyForm( userInfo, imaid, "0" );
        if ( null != imaList && !imaList.isEmpty() ) {
            rcilbForm = settingFormData( imaList, processId );
        }

        // 收集子单信息
        List<InvMatApplyDetailVO> imadList = invMatApplyDetailService.queryMatApplyDetailList( userInfo, imaid )
                .getResults();
        if ( null != imadList && !imadList.isEmpty() ) {
            rcilbList = settingListData( imadList, rcilbForm );
     ...
     * @return:
     */
    private RetContentInLineBean settingFormData(List<InvMatApplyVO> imaList, String processId) throws Exception {

        RetContentInLineBean pafem = new RetContentInLineBean();
        InvMatApplyVO ima = imaList.get( 0 ); // 获取到的表单数据

        pafem.setFoldable( true );// 是否可以折叠标志
        pafem.setIsShow( true ); // 是否显示标志
        pafem.setName( getPropertiesVal( "eip2invMatApplyFormGroupName" ) );// 名称
        pafem.setType( Type.KeyValue );// 类别，这里固定keyValue

        // 通过properties文件中定义的字段反射获取价值
        List<Object> rkvList = new ArrayList<Object>();
        String[] formField = getPropertiesVal( "eip2invMatApplyForm" ).split( "," );
        for ( String ffield : formField ) {
            String[] fieldContent = ffield.split( "#@#" );
            String fieldKey = fieldContent[0];
            String fieldVal = ReflectionUtil.getFieldValueByFieldName( ima, fieldContent[1] );
     ...
     * @throws Exception:
     */
    private RetContentInLineBean settingListData(List<InvMatApplyDetailVO> imadList, RetContentInLineBean rcilbForm)
            throws Exception {
        ...
        List<Object> rkvList = new ArrayList<Object>();
        // 获取column字段
        String[] listField = getPropertiesVal( "eip2invMatApplyList" ).split( "," );
        // 获取list数据
        for ( InvMatApplyDetailVO imad : imadList ) {
            ...
            for ( String lField : listField ) {
                ...
                String fieldKey = fieldContent[0];
                String fieldVal = ReflectionUtil.getFieldValueByFieldName( imad, fieldContent[1] );
                // 如果是itemname的话就做特殊处理
        ...
        List<Object> formList = rcilbForm.getValue();
        formList.add( new RetKeyValue( totalName, String.valueOf( totalPrice ) ) );
        rcilbForm.setValue( formList );
